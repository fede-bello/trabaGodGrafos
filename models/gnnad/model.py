...
class GraphLayer(MessagePassing):
    ...
    def message(self, x_i, x_j, edge_index_i, size_i, embedding, edges):
        ...
        ############ CHANGE TO APPLY GATV2  ##########################
        cat_att_i = torch.cat((self.att_i, self.att_em_i), dim=-1)
        cat_att_j = torch.cat((self.att_j, self.att_em_j), dim=-1)

        # Apply Leaky ReLU to concatenated attention values
        cat_att_i = F.leaky_relu(cat_att_i, self.negative_slope)
        cat_att_j = F.leaky_relu(cat_att_j, self.negative_slope)
...
class GNNAD:
    ...
    def predict_batch(self, X_train, X_test, y_test):
        # self._load_model()
        self.model.load_state_dict(torch.load(self.model_path))
        best_model = self.model.to(self.device)

        out = []
        for i, (x, y, _, edge_index) in enumerate(self.test_dataloader):
            x, y, edge_index = [
                item.float().to(self.device) for item in [x, y, edge_index]
            ]
            with torch.no_grad():
                out_batch = best_model(x).float().to(self.device).detach()
